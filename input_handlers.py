...
import actions
from actions import Action, BumpAction, EscapeAction, WaitAction, PickUpAction
import color
import exceptions
...
class EventHandler(BaseEventHandler):
    ...
    def handle_events(self, event: tcod.event.Event) -> BaseEventHandler:
        ...
        if self.handle_action(action_or_state):
            ...
            return MainEventHandler(self.engine)  # Return to the main handler.
        return self
...
class AskUserEventHandler(EventHandler):
    ...
    def on_exit(self) -> Optional[ActionOrHandler]:
        """Called when the user is typing to exit or cancel an action.
...
class SelectIndexHandler(AskUserEventHandler):
    ...
    def __init__(self, engine: Engine):
        """Sets the cursor to the plater when this handler is constructed."""
...
class AreaRangedAttackHandler(SelectIndexHandler):
    ...
    def on_render(self, console: tcod.console.Console) -> None:
        ...
        # Draw a rectangle around the targeted area, so the player can the affected tiles.
        console.draw_frame(
            x=x - self.radius - 1,
            y=y - self.radius - 1,
            width=self.radius ** 2,
...
class MainEventHandler(EventHandler):
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[ActionOrHandler]:
        ...
        elif key == tcod.event.KeySym.v:  # view
            return HistoryViewer(self.engine)
        elif key == tcod.event.KeySym.g:  # get
            action = PickUpAction(player)
        elif key == tcod.event.KeySym.i:  # inventory
            return InventoryActivateHandler(self.engine)
        elif key == tcod.event.KeySym.d:  # drop
...
class HistoryViewer(EventHandler):
    ...
    def on_render(self, console: tcod.console.Console) -> None:
        ...
        log_console.draw_frame(
            x=0, y=0, width=log_console.width, height=log_console.height
        )
